...
- `progress_report`：按 `notification.progress_interval` 周期发送进度快照，始终包含全部尚未成功的组件（已成功并已通过 `component_succeeded` 单独通知的组件由 `suppress_succeeded_in_progress` 控制是否隐藏，默认隐藏）
